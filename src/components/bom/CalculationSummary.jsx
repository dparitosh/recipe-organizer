...
export function CalculationSummary({ result, bom }) {
  if (!result) {
    return (
  ...
  const totalCost = result.costBreakdown.totalCost || 0
      ...
      <div>
        ...
        <div className="space-y-3">
          <Card className="p-3 bg-card/50">
            ...
            <Progress value={getPercentage(result.costBreakdown.materialCost, totalCost)} className="h-1.5" />
          </Card>
          ...
          <Card className="p-3 bg-card/50">
            ...
            <Progress value={getPercentage(result.costBreakdown.processingCost, totalCost)} className="h-1.5" />
          </Card>
          ...
          <Card className="p-3 bg-card/50">
            ...
            <Progress value={getPercentage(result.costBreakdown.overheadCost, totalCost)} className="h-1.5" />
          </Card>
          ...
          <Card className="p-4 bg-primary/10 border-primary/30">
            <div className="flex justify-between items-center">
              ...
              <p className="text-2xl font-bold text-primary">${result.costBreakdown.totalCost.toFixed(2)}</p>
            </div>
          </Card>
        </div>
      </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {getPercentage(result.totalWaste, totalInput).toFixed(1)}% of input
                </p>
              </div>
            </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      {byproduct.value !== undefined && (
                        <p className="text-xs text-muted-foreground">Value: ${byproduct.value.toFixed(2)}</p>
                      )}
                    </div>
                  </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {result.warnings.map((warning, index) => (
                <Card key={index} className="p-3 bg-amber-500/10 border-amber-500/30">
