...
import { Calculator, TrendUp, Package, Coins, Lightbulb } from '@phosphor-icons/react';
import { createCalculationEngine } from '@/lib/engines/calculationEngine';
import { toast } from 'sonner';

export const CalculationEngineInterface = ({ formulation, bom, onCalculationComplete }) => {
            ...
            <Card className="p-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span
                    className={`font-bold ${
                      result.metadata.efficiencyScore >= 80
                        ? 'text-green-500'
                        : result.metadata.efficiencyScore >= 60
                          ? 'text-yellow-500'
                          : 'text-red-500'
                    }`}
                  >
              ...
              {result.scaledIngredients.map((ing, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-secondary/20 rounded-lg">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">{ing.percentage.toFixed(2)}%</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {result.yieldChain.length > 1 && (
            <Card className="p-6">
              ...
              <div className="space-y-3">
                {result.yieldChain
                  .filter((step) => step.stepName !== 'Initial Input')
                  .map((step, idx) => (
...
export default CalculationEngineInterface;
