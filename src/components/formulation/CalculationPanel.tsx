...
import { Calculator, ChartBar, Scales, Recycle } from '@phosphor-icons/react'
import { Formulation } from '@/lib/schemas/formulation'
import { calculateYield, YieldParameters, YieldResult } from '@/lib/engines/yield'
import { calculateCost, CostParameters, CostResult } from '@/lib/engines/cost'
import { scaleFormulation, ScalingOptions } from '@/lib/engines/scaling'

interface CalculationPanelProps {
  formulation: Formulation
  onScaledFormulation?: (formulation: Formulation) => void
}

export function CalculationPanel({ formulation, onScaledFormulation }: CalculationPanelProps) {
  const [yieldParams, setYieldParams] = useState<YieldParameters>({
    processLoss: 5,
    moistureContent: 0,
    evaporationRate: 0,
    wasteFactor: 2
            ...
            {yieldResult && (
              <div className="space-y-3 p-4 bg-secondary/30 rounded-lg">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{yieldResult.theoreticalYield.toFixed(2)} {yieldResult.unit}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Actual Yield:</span>
                  <span className="font-semibold">{yieldResult.actualYield.toFixed(2)} {yieldResult.unit}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="text-xs space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span>{yieldResult.lossBreakdown.processLoss.toFixed(2)} {yieldResult.unit}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Moisture Loss:</span>
                    <span>{yieldResult.lossBreakdown.moistureLoss.toFixed(2)} {yieldResult.unit}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Total Loss:</span>
                    <span className="font-semibold">{yieldResult.lossBreakdown.total.toFixed(2)} {yieldResult.unit}</span>
                  </div>
                </div>
                  ...
                  <>
                    ...
                    <div className="space-y-1">
                      {yieldResult.warnings.map((warning, idx) => (
                        <p key={idx} className="text-xs text-orange-500">{warning}</p>
                      ))}
                    </div>
                  </>
                )}
            ...
            {costResult && (
              <div className="space-y-3 p-4 bg-secondary/30 rounded-lg">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-primary">${costResult.profitability.targetPrice?.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
          ...
          <TabsContent value="scale" className="space-y-4">
            ...
            <div className="p-4 bg-secondary/30 rounded-lg space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">{formulation.targetYield} {formulation.yieldUnit}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Scale Factor:</span>
                <span className="font-semibold">{(scaleTarget / formulation.targetYield).toFixed(2)}x</span>
              </div>
            </div>
