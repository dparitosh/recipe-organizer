import { Recipe, RecipeSchema } from '../agent-schemas'

export interface RecipeEngineerInput {
  userRequest: string
  context?: {
...
export interface RecipeEngineerOutput {
  recipe: Recipe
  reasoning: string
  alternatives?: string[]
}
...
export class RecipeEngineerAgent {
  ...
  async execute(input: RecipeEngineerInput): Promise<RecipeEngineerOutput> {
