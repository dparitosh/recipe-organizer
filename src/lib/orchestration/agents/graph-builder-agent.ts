import { Recipe, Calculation, GraphSchemaOutputType, GraphSchemaOutput } from '../agent-schemas'
...
export class GraphBuilderAgent {
  ...
  async execute(input: GraphBuilderInput): Promise<GraphBuilderOutput> {
    const prompt = window.spark.llmPrompt`You are a Graph Builder agent specializing in Neo4j graph data modeling.
