...
export class GenAIClient {
  ...
  async generateCypherWithSparkLLM(request) {
    try {
...
Cypher Query:`
      ...
      if (error.message && error.message.includes('Authorization')) {
        throw new Error('AI service authentication failed. Please try again later.')
      }
  ...
  async generateCypherWithNeo4jPlugin(request) {
  ...
  async executeCypherFromNL(naturalLanguageQuery, context) {
