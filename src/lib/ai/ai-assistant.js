...
class AIAssistantService {
  async query(request) {
    ...
    try {
      ...
      if (error.message && error.message.includes('Authorization')) {
        errorMessage += 'There was an authentication issue with the AI service. Please try again in a moment.'
  ...
  async analyzeQuery(question) {
    try {
      if (!window.spark || !window.spark.llmPrompt || !window.spark.llm) {
        throw new Error('Spark LLM API not available')
      }
...
`
  ...
  async retrieveGraphContext(request, analysis) {
...
`
  ...
  buildContextString(context, analysis) {
    const parts = []
  ...
  enrichContext(context, graphData) {
    return {
      ...
      activeFormulation: context?.activeFormulationId ? 
        ...
        undefined,
      graphNodes: graphData?.records?.map(r => r.toObject()) || [],
      timeRange: context?.timeRange
    }
  }

  async generateAnswer(question, context, analysis) {
...
`
      ...
      if (error.message && error.message.includes('Authorization')) {
        return {
          text: 'Unable to generate answer due to an authentication issue with the AI service. Please try again shortly.',
          confidence: 0
        }
      }
  ...
  extractNodeHighlights(graphData, analysis) {
  ...
  calculateRelevance(properties, analysis) {
    let score = 0.5
  ...
  extractRelationshipSummaries(graphData, analysis) {
    if (!graphData || !graphData.records) return []
  ...
  describeRelationship(type, count) {
    const descriptions = {
  ...
  async generateRecommendations(request, graphData, analysis) {
  ...
  async generateSubstitutionRecommendations(context) {
    const recommendations = []
  ...
  generateYieldImprovementRecommendations(context) {
    const recommendations = []
  ...
  analyzeFormulationIssues(formulation) {
    const recommendations = []
  ...
  identifySources(graphData, context) {
    const sources = []
