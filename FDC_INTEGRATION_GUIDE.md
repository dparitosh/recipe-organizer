...
### API Key
Set the FDC API key via environment configuration (for example `VITE_FDC_API_KEY` on the frontend and `FDC_API_KEY` on the backend).
```
[REPLACE_WITH_YOUR_FDC_API_KEY]
```
